...
const DISTRIBUIDOR_API = 'services/admin/distribuidor.php';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    MAIN_TITLE.textContent = 'Gestionar distribuidores';
...
SAVE_FORM.addEventListener('submit', async (event) => {
    ...
    const action = ID_Distribuidor.value ? 'updateRow' : 'createRow';
    // Constante tipo objeto con los datos del formulario.
    const FORM = new FormData(SAVE_FORM);
    // Petición para guardar los datos del formulario.
    const DATA = await fetchData(DISTRIBUIDOR_API, action, FORM);
...
const fillTable = async (form = null) => {
    ...
    if (DATA && DATA.status) {
            ...
            TABLE_BODY.innerHTML += `
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-warning" onclick="openReport(${row.id_Distribuidor})">
        ...
        sweetAlert(4, DATA ? DATA.error : 'Error desconocido', true);
    }
}
...
const openUpdate = async (id) => {
    ...
    const DATA = await fetchData(DISTRIBUIDOR_API, 'readOne', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con el error.
    if (DATA && DATA.status) {
        // Se muestra la caja de diálogo con su título.
        SAVE_MODAL.show();
        MODAL_TITLE.textContent = 'Actualizar distribuidor';
        // Se prepara el formulario.
        SAVE_FORM.reset();
...
const openDelete = async (id) => {
    ...
    if (RESPONSE) {
        ...
        const DATA = await fetchData(DISTRIBUIDOR_API, 'deleteRow', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con el error.
        if (DATA && DATA.status) {
            // Se muestra un mensaje de éxito.
...
const openReport = (id) => {
    ...
    const PATH = new URL(`${SERVER_URL}reports/admin/productos_Distribuidor.php`);
    // Se agrega un parámetro a la ruta con el valor del registro seleccionado.
    PATH.searchParams.append('idDistribuidor', id);
