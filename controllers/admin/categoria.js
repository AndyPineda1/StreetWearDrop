...
const openChart = async (id) => {
  ...
  if (DATA.status) {
    ...
    barGraph(
      ...
      "Top 5 de productos con más unidades vendidas"
    );
  } else {
    sweetAlert(4, DATA.error, true);
  }
};
const openChart1 = async () => {
  // Petición para obtener los datos del registro solicitado.
  const DATA = await fetchData(CATEGORIA_API, "readTopCategories");
  ...
  if (DATA.status) {
    ...
    // Verifica si DATA.dataset es un array y tiene elementos

    // Se recorre el conjunto de registros fila por fila a través del objeto row.
    DATA.dataset.forEach((row) => {
      productos.push(row.Categoria);
      unidades.push(row.Total_Vendidos);
    });
    ...
    barGraph(
      ...
      unidades,
      "Cantidad de productos",
      "Top 5 categorias mas vendidas"
    );
  } else {
    sweetAlert(4, "No hay datos disponibles para mostrar.", true);
  }
};
